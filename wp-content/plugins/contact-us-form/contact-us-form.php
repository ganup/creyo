...
Plugin URI:  https://wordpress.org/plugins/contact-us-form/
Text Domain: contact-us-form
Description: Generates a Contact Us Form in on a desired page, post, or sidebar of your website. Simply insert [CONTACT-US-FORM] within any post or page. 
Version:     1.2
...
class ContactUsForm
{
    ...
    function ContactUsForm()
    {
        ...
        $this->captcha = new ContactUsFormCaptcha( rand(1000000000, 9999999999) );
        ...
        if ( function_exists('register_uninstall_hook') )
            register_uninstall_hook(ABSPATH.PLUGINDIR.'/contact-us-form/contact-us-form.php', 'ContactUsForm:uninstall');
    ...
    function getSafeVar($name, $type='POST') {
        if($type == 'POST')
    ...
    function showForm( $params = '' )
    {
        ...
        $_SESSION['answer'] = $this->captcha->getAnswer();
        ...
        // Hidden settings flag no longer checked
        if ( $this->o['hideform'] == false || empty($result) || ($result != $this->o['msg_ok']))
        {
	        	unset($_POST['cuf_sender'.$n]);
				unset($_POST['cuf_email'.$n]);
				unset($_POST['cuf_subject'.$n]);
				unset($_POST['cuf_msg'.$n]);
				foreach ($_POST as $k => $dummy)
					if ( strpos( $k, 'cuf_field_') !== false )
						unset($k);
        	
            if ( !empty($_POST['cuf_subject'.$n]) )
                $cuf_subject = $this->getSafeVar('cuf_subject'.$n);
                ...
                $cuf_subject = $this->getSafeVar('subject', 'GET');
            ...
            $cuf_sender = (isset($_POST['cuf_sender'.$n])) ?  $this->getSafeVar('cuf_sender'.$n) : '';
            $cuf_email = (isset($_POST['cuf_email'.$n])) ?  $this->getSafeVar('cuf_email'.$n) : '';
            $cuf_msg = (isset($_POST['cuf_msg'.$n])) ?  $this->getSafeVar('cuf_msg'.$n) : '';
            ...
            {
                $i = 'cuf_field_'.$x.$n;
                $cuf_f = (isset($_POST[$i])) ? $this->getSafeVar($i) : '';
                $f = $this->o['field_'.$x];
                if ( !empty($f) )
                    $form .= '
            ...
            $form .= '
                ...
                ';
            if ( $this->o['captcha']==0 )
                $form .= $this->captcha->getCaptcha($n);
            if ( $this->o['captcha']==1 )
                $form .= '
    ...
    function addScript()
    {
        ...
        $script .= '
            ...
            if ( !isEmail(f[2]) )
                msg += "'.__('Please input right email.', 'cuf-lang').'";
            if ( msg != "" )
            {
                alert(msg);
                return false;
    ...
    function sendMail( $n = '', $params = '' )
    {
        ...
        if ( $result == 'OK' )
        {
            ...
            $name	= $this->getSafeVar('cuf_sender'.$n);
            $email	= $this->getSafeVar('cuf_email'.$n);
            $subject= $this->o['subpre'].' '.$this->getSafeVar('cuf_subject'.$n);
            $msg	= $this->getSafeVar('cuf_msg'.$n);
            ...
            $fullmsg =
                ...
                "Email: $email\r\n".
                $extra."\r\n".
                'Subject: '.$this->getSafeVar('cuf_subject'.$n)."\r\n\r\n".
                wordwrap($msg, 76, "\r\n")."\r\n\r\n".
    ...
    function optionsPage()
    {
        ...
        {
            $to = stripslashes($this->getSafeVar('cuf_to_email'));
            if ( empty($to) )
                $to = get_option('admin_email');
            $msg_ok = stripslashes($this->getSafeVar('cuf_msg_ok'));
            if ( empty($msg_ok) )
                $msg_ok = "Thank you! Your message was sent successfully.";
            $msg_err = stripslashes($this->getSafeVar('cuf_msg_err'));
            if ( empty($msg_err) )
            ...
            $this->o = array(
                'to_email'		=> $to,
                'from_email'	=> stripslashes($this->getSafeVar('cuf_from_email')),
                'css'			=> stripslashes($this->getSafeVar('cuf_css')),
                'msg_ok'		=> $msg_ok,
                'msg_err'		=> $msg_err,
                'submit'		=> stripslashes($this->getSafeVar('cuf_submit')),
								'captcha'		=> stripslashes($this->getSafeVar('cuf_captcha')),
                'captcha_label'	=> stripslashes($this->getSafeVar('cuf_captcha_label')),
                'captcha2_question'	=> stripslashes($this->getSafeVar('cuf_captcha2_question')),
                'captcha2_answer'	=> stripslashes($this->getSafeVar('cuf_captcha2_answer')),
                'subpre'		=> stripslashes($this->getSafeVar('cuf_subpre')),
                'field_1'		=> stripslashes($this->getSafeVar('cuf_field_1')),
                'field_2'		=> stripslashes($this->getSafeVar('cuf_field_2')),
                'field_3'		=> stripslashes($this->getSafeVar('cuf_field_3')),
                'field_4'		=> stripslashes($this->getSafeVar('cuf_field_4')),
                'field_5'		=> stripslashes($this->getSafeVar('cuf_field_5')),
                'color'			=> stripslashes($this->getSafeVar('cuf_color')),
				'size'			=> stripslashes($this->getSafeVar('cuf_size')),
                'hideform'		=> $hideform
            );
                ...
                <div class="inside">
                    ...
                    <form action="options-general.php?page=contact-us-form" method="post">
                        <table class="form-table">
                            ...
                            <tr>
                                <th>   </th>
	    	                    <td><?php _e('To insert the form on the page simply paste the following shortcode in the Editor at the desired location', 'cuf-lang') ?>: <b>[CONTACT-US-FORM]</b></td>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td><label for="cuf_captcha"><input name="cuf_captcha" type="radio" value="0"<?php if($this->o['captcha']==0) echo 'checked="checked"' ?> /> <?php _e('add two small numbers "2 + 5 ="', 'cuf-lang'); ?></label></td>
                            </tr>
                            <tr>
                                <th><?php _e('Captcha Label:', 'cuf-lang')?></th>
                                <td><input name="cuf_captcha_label" type="text" size="70" value="<?php echo $this->o['captcha_label'] ? $this->o['captcha_label'] : __('Are you a human:', 'cuf-lang') ?>" /></td>
                            </tr>
                            <tr style="border-top: 1px #ddd dashed;" >
                                <th><?php _e('Question Captcha:', 'cuf-lang')?></th>
                                <td><label for="cuf_captcha2"><input name="cuf_captcha" type="radio" value="1" <?php if($this->o['captcha']==1) echo 'checked="checked"' ?> /> <?php _e('Set you own question and answer.', 'cuf-lang'); ?></label></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
							<th><?php _e('Button Color:', 'cuf-lang'); ?></th>
							<td>
								<select name="cuf_color" style="width:200px;">
									<option <?php echo ((isset($this->o['color']) && $this->o['color'] == 'green') ? 'selected' : '');  ?> value="green"><?php _e('Green', 'cuf-lang'); ?></option>
									<option <?php echo ((isset($this->o['color']) && $this->o['color'] == 'blue') ? 'selected' : '');  ?> value="blue"><?php _e('Blue', 'cuf-lang'); ?></option>
									<option <?php echo ((isset($this->o['color']) && $this->o['color'] == 'grey') ? 'selected' : '');  ?> value="grey"><?php _e('Grey', 'cuf-lang'); ?></option>
								</select>
							</td>
						</tr>
						<tr>
							<th><?php _e('Form\'s Size:', 'cuf-lang'); ?></th>
							<td>
								<select name="cuf_size" style="width:200px;">
									<option <?php echo ((isset($this->o['size']) && $this->o['size'] == 'small') ? 'selected' : '');  ?> value="small"><?php _e('Small', 'cuf-lang'); ?></option>
									<option <?php echo ((isset($this->o['size']) && $this->o['size'] == 'medium') ? 'selected' : '');  ?> value="medium"><?php _e('Medium', 'cuf-lang'); ?></option>
									<option <?php echo ((isset($this->o['size']) && $this->o['size'] == 'large') ? 'selected' : '');  ?> value="large"><?php _e('Large', 'cuf-lang'); ?></option>
								</select>
							</td>
						</tr>
                        </table>
    ...
    function addOptionsPage()
    {
		...
		add_options_page('Contact Us Form', $menutitle, 'edit_pages', 'contact-us-form', array( &$this, 'optionsPage'));
    }
    ...
    function checkInput( $n = '' )
    {
        ...
        $_POST['cuf_sender'.$n] = stripslashes(trim($this->getSafeVar('cuf_sender'.$n)));
        $_POST['cuf_email'.$n] = stripslashes(trim($this->getSafeVar('cuf_email'.$n)));
        $_POST['cuf_subject'.$n] = stripslashes(trim($this->getSafeVar('cuf_subject'.$n)));
        $_POST['cuf_msg'.$n] = stripslashes(trim($this->getSafeVar('cuf_msg'.$n)));
        ...
        if ( $o['captcha']==0 && !$this->captcha->isCaptchaOk() )
            $error[] = $this->o['captcha_label'];
        if ( $o['captcha']==1 && ( empty($_POST['cuf_captcha2_'.$n]) || $_POST['cuf_captcha2_'.$n] != $o['captcha2_answer'] ) )
            $error[] = $this->o['captcha2_question'];
        if ( !empty($error) )
            return __('Check these fields:', 'cuf-lang').' '.'<font color="red">'.implode(', ', $error).'</font>';
...
."</style>\n";
    ...
    function setRecources()
    {
        if ( isset($_GET['resource']) && !empty($_GET['resource']) )
        {
            ...
            {
                $content = base64_decode($resources[$this->getSafeVar('resource', 'GET')]);
                ...
                else
                {
                    ...
                    header('Content-Type: image/' .  substr(strrchr($this->getSafeVar('resource', 'GET'), '.'), 1) );
                    echo $content;
                    exit;
                }
            }
...
class ContactUsFormCaptcha
{
    ...
    function isCaptchaOk()
    {
        ...
        if ($_POST[base64_encode(strrev('current_time'))] && $_POST[base64_encode(strrev('captcha'))])
        {
            ...
            if ($_POST[base64_encode(strrev('answer'))] != $_SESSION['answer'])
                $ok = false;
        }
        return $ok;
    }
...
class ContactUsForm_Widget extends WP_Widget
{
    ...
    function __construct() {
        parent::__construct('cuform_widget', 'Contact Us Form', array('description' => 'Contact Us Form'));
    }
