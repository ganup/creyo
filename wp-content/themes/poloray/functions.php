...
require_once ($poloray_functions_path . 'customizer.php');
require_once ($poloray_functions_path . 'themes-page.php');
/* ----------------------------------------------------------------------------------- */
/* jQuery Enqueue */
/* ----------------------------------------------------------------------------------- */
 ...
 */
function poloray_migrate_option() {
    if (get_option('poloray_options') && !get_option('poloray_option_migrate')) {
        $theme_options = array('poloray_logo', 'poloray_favicon', 'poloray_slideimage1', 'poloray_wimg1', 'poloray_fimg2', 'poloray_fimg3', 'poloray_fimg4');
        $wp_upload_dir = wp_upload_dir();
        require ( ABSPATH . 'wp-admin/includes/image.php' );
        foreach ($theme_options as $option) {
            $option_value = poloray_get_option($option);
            if ($option_value && $option_value != '') {
                ...
                poloray_import_file($new_image_url);
            }
        }
        update_option('poloray_option_migrate', true);
    }
}
add_action('init', 'poloray_migrate_option');
/**
 * Import Files From Uploads To Attachment
 */
function poloray_import_file($file, $post_id = 0, $import_date = 'file') {
    set_time_limit(120);
...
//     A writable uploads dir will pass this test. Again, there's no point overriding this one.
    ...
    if ((!$type || !$ext ) && !current_user_can('unfiltered_upload')) {
        return new WP_Error('wrong_file_type', __('Sorry, this file type is not permitted for security reasons.', 'poloray')); //A WP-core string..
    }
    ...
    if (preg_match('|^' . preg_quote(str_replace('\\', '/', $uploads['basedir'])) . '(.*)$|i', $file_name, $mat)) {
        ...
        if (!empty($attachment)) {
            return new WP_Error('file_exists', __('Sorry, That file already exists in the WordPress media library.', 'poloray'));
        }
        ...
        if (false === @copy($file, $new_file))
            return new WP_Error('upload_error', sprintf(__('The selected file could not be copied to %s.', 'poloray'), $uploads['path']));
